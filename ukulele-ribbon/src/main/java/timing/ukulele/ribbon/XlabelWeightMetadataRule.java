...
import static timing.ukulele.ribbon.RibbonConstant.*;
...
public class XlabelWeightMetadataRule extends ZoneAvoidanceRule {
    ...
    public Server choose(Object key) {
        ...
        for (Server server : serverList) {
            ...
            // labelOr 或
            String labelOr = metadata.get(LABEL_OR);
            if (!StringUtils.isEmpty(labelOr)) {
                String[] metadataLabel = labelOr.split(HEADER_LABEL_SPLIT);
            ...
            // labelAnd 且
            String labelAnd = metadata.get(LABEL_AND);
            if (!StringUtils.isEmpty(labelAnd)) {
                List<String> metadataLabel = Arrays.asList(labelAnd.split(HEADER_LABEL_SPLIT));
                if (XlabelMvcHeaderInterceptor.LABEL.get().containsAll(metadataLabel)) {
                    return server;
                }
            }
            // 根据权重做选择
            String strWeight = metadata.get(WEIGHT_KEY);
            // 默认 100
            int weight = 100;
            try {
